...
namespace CMS_Entity.Mapping
{
    public class CMS_ImagesMap : EntityTypeConfiguration<CMS_Images>
    {
        public CMS_ImagesMap()
        {
            ...
            this.HasOptional(x => x.Categories).WithMany(x => x.Images).HasForeignKey(x => x.CategoryId);
            this.HasOptional(x => x.Collections).WithMany(x => x.Images).HasForeignKey(x => x.CollectionId);
            ...
            this.Property(x => x.CollectionId).HasMaxLength(60).HasColumnType("varchar").IsOptional();
