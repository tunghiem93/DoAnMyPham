        ...
        public bool CreateOrUpdate(CMSBrandsModels model, ref string Id, ref string msg)
        {
            ...
            using (var cxt = new CMS_Context())
            {
                using (var beginTran = cxt.Database.BeginTransaction())
                {
                    try
                    {
                        var _IsExits = cxt.CMS_Brands.Any(x => (x.BrandCode.Equals(model.BrandCode) || x.BrandName.Equals(model.BrandName)) && (string.IsNullOrEmpty(model.Id) ? 1 == 1 : !x.Id.Equals(model.Id)));
                        if (_IsExits)
                        {
                            result = false;
